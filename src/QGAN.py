...
def elementary_layer(prefix = "", num_qubits=2):
    ...
    for i in range(num_qubits):
        ...
        layer.ry(f"ry-{i}", i)
        layer.rz(f"rz-{i}", i)
    layer.un(X, [(i+1) % num_qubits for i in range(num_qubits)], [i for i in range(num_qubits)])
...
def Generator(backbone):
    ...
    hams = [Hamiltonian(QubitOperator(f"Z{i}")+QubitOperator("")) for i in range(num_qubits)]
    simulator = Simulator("projectq", n_qubits=num_qubits)
    grad_ops = simulator.get_expectation_with_grad(hams=hams, circ_right=backbone)
